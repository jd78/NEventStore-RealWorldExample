...
using FootballMatchStatEventStore.Ex;
using NEventStore;
        ...
        public void Save(string bucketId, IAggregate aggregate, Guid commitId, Action<IDictionary<string, object>> updateHeaders)
        {
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    var takeSnapshot = aggregate.GetUncommittedEvents().Cast<IEvent>().Any(p => p.Version >0 && p.Version%6 == 0);
                    if (takeSnapshot)
                    {
                        Task.Run(() =>
                        {
                            _eventStore.Advanced.AddSnapshot(new Snapshot(stream.StreamId, stream.StreamRevision, MatchMemento.Create(aggregate)));
                        });
                    }
